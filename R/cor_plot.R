...
#' @param return_significant_only Whether the returned values (table, not the plot) should only contain the correlations that are significant. Defaults 
#' to FALSE. If set to TRUE, will return any correlation with a p value smaller than the largest supplied significance level. For example, if sign_levels is not modified,
#' any correlation with p < .1 will be returned. Only works if significance is set to TRUE.
#' @param CI Whether the CI for each correlation should be reported in the table. Defaults to TRUE.
#' @param CI_conf_level Confidence level used for the CI. Defaults to 0.95.
...
cor_plot <- function(data, x, y = NULL, use = "complete.obs", method = "pearson", digits = 3, digit_size = 4,
  ...
  if(CI){
    ...
    CIs <- data.frame(t(sapply(cor_plot_data$value, function(x) psychometric::CIr(x,n, level = CI_conf_level))))
  ...
  if (plot_CI){
    if(!CI){
      warning("CI is set to false, cannot add it to plot")
    } else {
  ...
  if(return_significant_only){
    if(!significance){
      warning("significance is set to FALSE, cannot remove non-significant correlations.")
    } else {
      thresh <- as.numeric(max(names(sign_levels)))
      cor_plot_data <- filter(cor_plot_data, p_val < thresh)
    }
  }
